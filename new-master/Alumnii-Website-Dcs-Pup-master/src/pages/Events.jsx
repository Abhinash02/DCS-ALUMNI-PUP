...
const Events = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError("");
      e.target.reset();
      setCurrentPage(1); // Reset to first page after adding a new event
    } catch (error) {
  ...
  const sliderSettings = {
    ...
    arrows: true,
    customPaging: () => (
      <div className="size-2.5 bg-darkBlue rounded-full hover:bg-lightBlueAlt transition-colors duration-200" />
    ),
    appendDots: (dots) => (
      <div>
  ...
  const PaginationControls = () => (
    <div className="flex justify-center items-center mt-8 space-x-2">
      <button
        ...
        className={`px-4 py-2 rounded-md font-poppins text-sm xs:text-base ${
          currentPage === 1
            ? "bg-gray-200 text-gray-500 cursor-not-allowed"
            : "bg-darkBlue text-white hover:bg-darkBlueAlt"
        }`}
      >
        Previous
      </button>
      {Array.from({ length: totalPages }, (_, i) => i + 1).map((number) => (
        <button
          ...
          className={`px-3 py-1 rounded-md font-poppins text-sm xs:text-base ${
            currentPage === number
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <input
                type="text"
                name="textColor"
                value={formData.textColor} // Fixed: Use textColor instead of title
                onChange={handleInputChange}
